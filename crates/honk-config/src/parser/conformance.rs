...
pub fn project(input: &str) -> Projection {
    ...
    let unknown_positions: Vec<_> = structural.iter()
        .filter(|d| d.code == "unknown-block")
        .filter_map(|d| d.span.clone()).collect();
    let document = parsed.ok().map(|document| {
        ...
        let sections: Vec<_> = document.sections()
            .map(|section| section_value(&section, section.header(), &mut sink)).collect();
    ...
    Projection {
        ...
        diagnostics: diagnostics.into_iter().map(|d| Diagnostic {
            code: d.code, setting: d.setting.to_string(), message: d.message,
        }).collect(),
        document,
        unknown_positions,
    }
...
fn section_value(section: &Segment<'_, '_>, context: &str, sink: &mut ParserDiagnostics<'_>) -> Value {
    ...
    if context == "routing" {
        ...
        for (end, line) in lines.iter().enumerate() {
            for (_, byte) in line.parentheses() {
                depth = if byte == b'(' { depth + 1 } else { depth.saturating_sub(1) };
            }
            if depth == 0 {
            ...
            for child in body {
                ...
                if child.is_block() {
                    ...
                    let next = match (context, name) {
                        ...
                        ("experimental", "clash_api" | "cache_file" | "udp_nfqueue") => Some("settings"),
                        _ => None,
                    };
...
fn statement(text: Expression<'_, '_, '_>, rules: bool) -> Option<Value> {
    ...
    let (val, functions) = if let Some(functions) = functions {
        ...
        (value.split(",").map(|v| v.unquote().value().into_owned()).collect::<Vec<_>>().join(","), Vec::new())
    };
    Some(json!({"param": {"key": key, "val": val, "and_functions": functions, "annotation": annotation}}))
}
...
fn key_value<'p, 'd, 'a>(text: Expression<'p, 'd, 'a>) -> (String, Expression<'p, 'd, 'a>) {
    if let Some(colon) = text.find(":") {
        ...
        if !text.sub(colon, text.span.end).starts_with("://") {
            return (text.sub(text.span.start, colon).unquote().value().into_owned(), text.sub(colon + 1, text.span.end).trim());
        }
    }
...
fn without_annotation<'p, 'd, 'a>(text: Expression<'p, 'd, 'a>) -> (Expression<'p, 'd, 'a>, Vec<Value>) {
    if let Some(open) = text.find("[")
        && let Some(close) = text.find("]")
        && close > open
        && text.sub(close + 1, text.span.end).trim().is_empty()
    {
        return (text.sub(text.span.start, open).trim(), parameters(text.sub(open + 1, close)));
    }
    (text, Vec::new())
}
...
fn function(text: Expression<'_, '_, '_>) -> Option<Value> {
    ...
    let text = if not { text.sub(text.span.start + 1, text.span.end).trim() } else { text };
    let open = text.find("(")?;
    let close = text.find(")")?;
    if close < open || !text.sub(close + 1, text.span.end).trim().is_empty() { return None; }
    Some(json!({"name": text.sub(text.span.start, open).trim().value(), "not": not, "params": parameters(text.sub(open + 1, close))}))
}

fn outbound(text: Expression<'_, '_, '_>) -> Value {
    function(text).unwrap_or_else(|| json!({"name": text.unquote().value(), "not": false, "params": []}))
}
