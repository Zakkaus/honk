...
Stage 1 accepts these value names and canonical units: `test_inventory` (`test-names`), `ignored_tests` (`tests`), `slowest_test` (`seconds`), `dns_smoke` (`queries-and-names`), `smoke_memory` (`MiB`, the release `honk-core` measured by the `smoke` lane), `smoke_cpu` (`seconds`), `reload_benchmark` (`ratios`), `toolchain` (`rustc-cache`), and `vm_environment` (`kernel-accelerator`). The ordinary layout fixture also carries `ebpf_instructions` (`instructions`) to reproduce the agreed comment; no stage-1 producer emits it.
