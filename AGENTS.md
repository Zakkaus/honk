...
- Environment variables: `RUST_LOG`; `HONK_API_SECRET` (`honk-tool diagnose` Clash API Bearer token, overridden by `--secret`); `HONK_UI_DOWNLOAD_URL` (highest-precedence UI ZIP URL override); `HONK_POOL_DISABLE=1` (bypass pool); `HONK_MI_COLLECT_SECS` (mimalloc-only per-owner park/sweep interval, default 60s, `0` disables).
...
- `diagnose [--api URL] [--secret TOKEN] [--pin-root PATH]` — one-shot read-only health check: engine process, `daens`/`dae0` presence, daens fwmark rule, pinned maps present, occupancy/overflow, and a 2xx Clash API response. `--api` accepts `http://` and `https://`. Non-2xx responses and five-second request timeouts (including the response body) fail the API check. Any failed check exits `1` with one `N issue(s) found` error; all checks passing prints `all checks passed` and exits `0`. `--secret` is visible in the process list, so prefer `HONK_API_SECRET`; either way, the token is sent in cleartext over `http://`.
