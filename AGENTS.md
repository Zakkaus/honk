...
Outbound source ownership: `proxy/{error,packet,outbound,registry}.rs` retain the public proxy facade; Shadowsocks owns `aead2022`/`stream`, and VLESS owns feature-only `handler`/`encryption` plus `cool`/`mux` backends. Shared UoT, transport and REALITY stay outside VLESS. QUIC owns `path_health`, `flow_control`, `metrics`, `endpoint`, `client`, `stream` and `boring`; the public `quic_boring` facade remains supported. AnyTLS uses `padding`/`writer`/`overflow`, Score uses `evidence`/`ranking`/`feedback`/`verification`, alive uses `health`/`urltest`, and SessionPool uses `maintenance`/`speculative`; common state stays at each ancestor to avoid widening fields. Normal builds without `rprx` allocate no VLESS pools/carrier semaphores; `cfg(test)` retains backend-only unit coverage, never handler registration.

Shared gRPC lives in `proxy/transport/grpc.rs`: `h2` owns HTTP/2 and connection-wide HPACK with a 64 KiB response-header limit and its default 4 KiB dynamic table. The returned stream owns the driver/socket; split readers and writers share stable driver wakeups. At most one 16 KiB application message is queued, and flush waits for both its ownership release and physical I/O flush, including after peer window reductions. Preserve buffered payload before terminal read errors, retained read outcomes, request half-close, and graceful GOAWAY for an admitted stream; never replay application bytes.

Score keeps business reliability separate from event-timed performance and configured-probe cohorts; never add overlapping global/family/exact completions. Live accepted progress must not multiply terminal outcomes or re-add final byte totals. Neutral cells stay in bounded LRUs to preserve in-flight incarnations and trial recency; admitted business reporters survive reload, but probe baselines are invalidated and old probe publication is fenced. TCP establishment orchestration stays in `control/connection/tcp.rs`, dial/admission in `tcp/dial.rs`; one Score replacement shares the original deadline and recorded final-edge provenance, never display-chain authorization or application replay. Score tests belong under `group/score/tests/`.
...
Score ordinary incumbent protection is separate from verification. Keep first-choice utility and existing trial budgets; an ineligible or unresolved-failure incumbent may escape without waiting. Recovery uses genuine targeted Traffic RX after setup/TX or a successful bidirectional terminal's actual RX, strictly newer than each applicable cell failure. Live publication is event-driven and capped at once per second per reporter; no timer guarantees delivery of throttled progress. Existing outcome counts, streaks/backoff and terminal verification remain independent. Fresh business RX can retain already earned four-completion qualification for 60 seconds from RX time; below-threshold expired/invalidated leases cannot be revived by RX alone. Failure/reload invalidation and started-cell incarnation fences remain authoritative; neutral cancellation does not retract factual RX. Hold maturity and reliability blending still use actual decayed counts without overlapping sums. Healthy promotion compares qualified common metric pairs against a fixed incumbent; expiry alone is not a performance advantage. `ordinarySwitch` excludes first choice/trials/Peek and is distinct from `switchFlap`. Do not add per-packet logs or exported target identities.

Score carrier pressure is advisory node-to-proxy evidence, never business outcome, target performance or ordinary UDP loss. Runtime-owner weak binding and active-authority/publication-time fences own admission. Physical TCP uses length-checked borrowed `TCP_INFO` with event-driven one-second gating; QUIC reuses its existing monitor after confirmation/publication and resets peer/counter baselines. Shared carriers publish sustained episodes, not one event per logical flow. Health-filter family is not socket identity; hints reopen node-scoped questions only for carrier-backed networks, excluding native Shadowsocks UDP and unobserved raw SOCKS/direct paths. Preserve validation budgets, backoff, utility, qualification, pool liveness, Vision Direct and terminal/health semantics. `carrierPressure` and `carrierValidation` are readonly diagnostic counters, not loss rates.
`carrierRttPressure` and `carrierLossPressure` count the reason at fresh-hint admission; a combined episode increments both but only one `carrierPressure`. Duplicate/stale hints add no cause counts. These are overlapping diagnostics, not transport identities, packet loss rates or additional business failures.
