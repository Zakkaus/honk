...
- **UDP transport invariant:** production endpoints use `PacketTransport`. Tunnel handlers frame datagrams directly: trojan, AnyTLS/VLESS UoT v2, VLESS H2MUX native connected UDP, VLESS XUDP/Mux.Cool, SS, TUIC/Hy2, and Juicity. Direct and SOCKS5 hide raw sockets behind the same contract; no loopback bridges remain. `UdpEndpointPool` retires dead transports rather than black-holing flows.
  ...
  Authoritative and cold URLTest UDP transport preparation share one absolute `max(10s, 4 × connect_timeout)` deadline, including the finalized winner's commit. Deadline expiry cancels and drains started preparations before returning; cancellation and never-started candidates are health-neutral, while already-completed errors still count. Endpoint publication and the separately bounded first send follow successful preparation.
