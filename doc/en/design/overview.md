...
2. The [DNS path](./dns.md) sends TCP and UDP destination port `53` through a fast path that skips the general match loop and redirects to the control plane. Transparent port-53, optional host-netns `dns.bind` UDP/TCP, and flow-associated reality/target lookups share `DnsController` → singleflight → `DnsForwarder`: source-aware request routing → cache → upstream pool (UDP/TCP/DoT/DoH/DoQ/DoH3, optional proxy leaf) → response routing. Answers project into global, source-independent `DOMAIN_ROUTING_MAP` for subsequent eBPF IP routing. Transparent/standalone `source_ip` is the socket peer; only transparent supplies `original_dst`. Flow lookups use admitted client IP, without intercepted DNS destination.
