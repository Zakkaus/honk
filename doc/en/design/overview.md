...
- Parser and share-link data entrypoints retain safe diagnostics without logging. Scalar values, names, links and raw error payloads are withheld; static failure causes and migration guidance remain available, including replacing standalone `mux` with `vless_mode` and `dns.hosts_file` with `dns.use_host`. Group/filter/subscription/entry positions use original ordinals. Detailed dae diagnostics share a metadata-only source table with include-parent links and available physical line numbers. Section traversal order is retained; acceptance changes are documented in the dialect and node references. Only the outer attempt appends a terminal cause.
- `src/node/validation.rs` owns intrinsic and collection admission, reusing the VLESS and ALPN checks in `src/node/protocol.rs`. Validation carries static field/cause data and allocates diagnostic context only on failure. Detailed Config loaders and collection admission retain that cause with the original one-based node ordinal; included semantic failures retain their field source before legacy category projection. Node-entry warnings and early share-link failures receive their physical entry coordinates at the parser boundary, independently of node tags. `Config::validate` projects `Config::validate_detailed`; DNS reference validation continues to share `DnsRouting::request_source` with the runtime and constructs diagnostic paths only on rejection.
- `src/parser/` — `mod.rs`: file-aware `include`, shared section helpers, global/node/group/subscription/experimental parsing, `resolve_group_filters`. `dns.rs`: DNS endpoints/upstreams/routing/fixed TTL; `routing.rs`: multiline routing statements/actions; `tests.rs`: combined parser. Includes resolve entry-relative globs inside the canonical entry directory; reject duplicates/cycles.
- `src/share_link.rs` — sole parser `Node::from_share_link`: SIP002 `ss://`, v2rayN/Shadowrocket `vmess://`, canonical/Shadowrocket VLESS, trojan/AnyTLS/Hy2/TUIC/Juicity/SOCKS5, transports, REALITY, ECH, pins, protocol tuning. Normalize authority, build credentials, apply query/TLS/transport/protocol options via `src/share_link/options.rs`, then validate/derive identity.
  Canonical VLESS: `vless_mode=legacy|uot-v2|h2mux|h2mux-padded|xudp|mux-cool`; omission preserves legacy IDs/behavior, `packetEncoding=xudp` maps to `xudp`. Reject ambiguous third-party mux/UoT/XUDP/packetaddr queries; never guess. Explicit `security=` overrides canonical VLESS's historical TLS-on default; encoded Shadowrocket defaults to plaintext without security options. `security=reality` or Shadowrocket `pbk`/`sid`/`spx` selects REALITY; `flow=`/`xtls=2` selects Vision. Hy2 accepts `hy2://`, preserving full `user:password`. Port hopping: `mport`/`mhop` or official `:p1,p2-p3`, never both; full list is hop set, first port nominal. Default names never expose credentials; chain links parse only first hop.
- VLESS share links map `encryption=` and the normalized mode before deriving identity. Clash subscriptions map enabled `smux`/`multiplex` H2MUX only when `protocol: h2mux` or an explicit `padding` boolean disambiguates it; enabled blocks missing both are rejected. `udp-over-tcp` version 0/2 maps to UoT v2; `packet-encoding: xudp`, `xudp: true`, or `udp: true` without another packet declaration maps to Single XUDP. Conflicts, packetaddr, and unsupported mux protocols/tuning are rejected. Mux.Cool requires the canonical share-link mode.
...
2. The [DNS path](./dns.md) sends LAN TCP and UDP destination port `53` through an early fast path that skips the compiled traffic policy; WAN routing retains its non-must port-53 control-plane override. Transparent port-53, optional host-netns `dns.bind` UDP/TCP, and flow-associated reality/target lookups share `DnsController` → generation-pinned `DnsService` → source-aware request routing → cache/singleflight → upstream pool (UDP/TCP/DoT/DoH/DoQ/DoH3, with an optional proxy leaf) → response routing. Accepted answers feed the generation-owned routing projection and domain-fact maps, which remain global and source-independent within that policy generation. Transparent and standalone `source_ip` is the socket peer; only transparent interception supplies `original_dst`. Flow lookups use the admitted client IP without an intercepted DNS destination.
...
5. The [control plane](./control-plane.md) recovers the original destination (`SO_ORIGINAL_DST` / `IP6T_SO_ORIGINAL_DST` for TCP with transparent-`local_addr` fallback; `IP_RECVORIGDSTADDR` cmsg for UDP) and consumes the eBPF routing handoff with `routing_handoff_take`. A missing handoff or `ControlPlaneRouting` outcome enters `Router::route_with_must`. eBPF-decided `direct` with nonzero mark offloads without userspace relay.
...
7. The [group layer](./groups.md) applies the Clash mode override without changing final `must`/`block` results, then `SharedGroupManager` resolves the authoritative group policy pick to a leaf node. When explicitly selected, Score ranks only health-eligible members from per-target TCP/UDP and target-family transport-quality scores; service-specific semantic unlock is instead modeled by routing or geosite selection of a dedicated Score group. TCP/UDP, including Score, normally use one authoritative leaf. Only cold top-level URLTest initially staggers contenders; only its winner binds the `PacketTransport` driver.
8. The [outbound layer](./outbound.md) dials that leaf through `TcpOutbound` (pool: ready → bare → fresh) and relays TCP or datagrams. Sniffed TCP bytes are forwarded first, then plain TCP uses splice and wrapped streams use copy; honor `src/relay/`'s idle `DRAIN_DEADLINE`.
