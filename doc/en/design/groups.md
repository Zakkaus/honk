...
| Probe path | Behavior |
| --- | --- |
| TCP | Sends the configured HTTP method to `tcp_check_url` through the node (200–499 is healthy in `ProxyHttpProber`), or performs a raw TCP connect when no HTTP probe applies. Reuses generation state only when `NodeRuntime::is_warm_or_stateless`; a cold reusable node first establishes its session/client in a guarded throwaway runtime and closes it deterministically. Setup and a throwaway HEAD are untimed; the second request uses the configured method and closes the connection. If the server closes early, timing falls back to the warm exchange. Only a completed HTTP exchange records warm-path RTT in the matching TCP family state. Setup and target-exchange failures both update liveness/cooldown without contributing latency or ranking strikes. |
...
- `src/urltest.rs` — Clash delay endpoints dial through node handlers; HTTPS uses real TLS via `tls::build_http_probe_connector` (`h2,http/1.1`). Negotiated ALPN selects HTTP/1.1 HEAD or an `h2` session, supporting h2-preferring gstatic. Status 200–499 is healthy; group concurrency caps at 10. **Every protocol reports one warm-path round trip:** proxy dial, target TLS, and first throwaway request are untimed setup; measure the second established-connection request. Failure/timeout falls back to the first exchange's time. Reuse warm generation state; cold AnyTLS, VLESS H2MUX/Mux.Cool, Hysteria2, TUIC, and Juicity warm in throwaway runtimes and close afterward, retaining no new per-node state. Each phase gets the full requested timeout: transport warm-up, dial, target TLS, warm-up request, measured request. Failures feed dial-failure streaks. Empty URL becomes `https://www.gstatic.com/generate_204`.
