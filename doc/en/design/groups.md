...
Business attempt and outcome evidence decays with a 30-minute half-life. Setup failures carry extra weight in the Beta confidence bounds. Qualified candidates need four effective useful completions, compatible reliability bounds and comparable observed failure risk; sparse candidates receive bounded trials. Once qualified, utility uses observed reliability rather than rewarding the most-sampled node's lower bound. Normal ranking, metric baselines, incumbent eligibility and winner reasons use the same eligible set. Overlapping global/family/exact completion evidence is never added as independent samples.
...
Recent usability is dated by the latest positive business RX, never by TX-only progress or terminal cleanup. Late completions retain their once-only lifetime outcome but cannot contribute expired RX or RX at or before the latest failure/reload fence. Unexpired out-of-order completions preserve the newest observation clock, and an admitted surviving flow may contribute genuinely new RX after that fence.
