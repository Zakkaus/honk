...
Business reliability retains its 30-minute half-life. Performance confidence ages independently and expires after 120 seconds regardless of sample count. Setup and first response publish immediately; accepted transfer progress uses nonoverlapping 1–10 second windows with at least 64 KiB in the measured direction, bidirectional flow progress and a response. Windows never multiply terminal successes or failures. Goodput is workload-dependent, not link capacity; silence is not congestion. Mature switching protection is `0.005` after eight effective completions, with fresh failure bypass. These experimental constants are not configurable; see [the lifecycle design](../design/groups.md#score-scoring-and-lifecycle).
