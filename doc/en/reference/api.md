...
Connection `outbound` is the routing-time group/action, not a current leaf or a reconstructed selection. `chain` holds the root-first group/leaf IDs captured at dial time, empty for direct/block and unknown once a captured name no longer resolves; `started_at` is the connection's registration time. With recording enabled, `flow_id`, first-observation UTC, domain provenance and the rule link to retained evidence, which takes precedence; missing/evicted evidence remains unknown, and without recording the rule is unknown. Captured userspace rule IDs share the generation identity used by `/rules` and trace; kernel-final provenance is not reconstructed. Per-connection rates remain null. Empty lists have `visibility: partial`, not proof that the device has no connections. HTTP readiness is not datapath readiness.
