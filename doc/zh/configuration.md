...
- 引号外、词元开头的 `#` 引入注释；裸值内的 `#` 保留为数据。条目读取器也接受链接结束引号或订阅 `(UA)` 后缀之后紧贴的 `#` 注释，并产生 `legacy-glued-hash` 警告；注释前应留空白。词元开头的注释内，花括号不会闭合配置块。
...
只有顶层 `include` 允许起始花括号另起一行，并产生 `legacy-include-opener` 警告；建议在同一行写 `include {`。include 注释遵循词元规则：`path.dae # note` 加载 `path.dae`，而 `path.dae#note` 是字面 glob 模式，并产生 `legacy-include-hash` 警告。字面 `#` 可加引号，以免触发迁移警告。字符串解析检查 include 结构，但不打开被包含文件；include 不能把片段拼接到尚未闭合的块中。
