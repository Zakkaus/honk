...
命中还要置 `domain_final`；其他类别在第一条用到它的规则入口、在该规则任何条件分支
之前解析，因此每条规则入口处已解析的类别集合在所有路径上相同，被更早规则决定的流
不做这次 lookup。map miss、缺失 MAC 和非法 family 都算已解析（区域填零）。这些区域
只在一次调用内有效，不跨 generation。

若改为在首次实际到达的使用点解析、用寄存器里的 readiness mask 守卫，每次决策至多
省三次 lookup，却把随路径变化的状态带回 verifier：有条件解析让不同路径上的已解析类别
mask 各不相同，readiness 测试又让它们保持精确且存活，mask 不同的状态互不吸收，后面的
进程名链就要按不同 mask 重走。Linux 6.12.107 上实测：#280 策略那样处理 160,415 条
指令，规则入口解析为 53,380；四条事实规则排在二十九条进程名规则前的策略为 368,874 对
100,191。所以采用规则入口解析；路由评估只在流缓存 miss 时执行，普通缓存报文不经过，
多出的 lookup 有界且少见。
...
精确值，无法吸收后来的未知值。同一内核上实测，这份策略去掉两条规则时处理 53,279
条指令，加上时为 53,380。verifier-budget 测试通过只证明可加载，不证明该指令数。
IPv4/IPv6 仍分别构造 key、
