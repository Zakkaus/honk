...
`.github/workflows/ci.yml` runs fmt + clippy, the workspace nextest gate, and an Ubuntu hosted-VM eBPF job. The host suites use the `ci` profile in `.config/nextest.toml`, with per-test timeouts and failure JUnit artifacts. The VM job boots the pinned Linux 6.12 image, BTF-checks the kernel object, installs the geo assets required by eBPF-feature routing tests, and runs the full `honk-core --features ebpf --lib` nextest gate on the host. `.github/ci/run-vm-gate.sh floor` then mounts bpffs in the guest and runs the real NFQUEUE/nftables netns contract, TC/cgroup link lifecycle, pinned allocator rollback-compatibility test, and root-only network tests serially without a job container.
